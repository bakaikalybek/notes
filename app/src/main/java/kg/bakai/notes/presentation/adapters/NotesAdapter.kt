...
class NotesAdapter(private val callback: (note: Note) -> Unit): RecyclerView.Adapter<NotesAdapter.NoteViewHolder>() {
    ...
    override fun onBindViewHolder(holder: NoteViewHolder, position: Int) {
        ...
        holder.bind(notes[position], mColors[position])
    }
    ...
    fun submitItems(list: List<Note>) {
        notes.clear()
        notes.addAll(list)
    ...
    inner class NoteViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        private val main: CardView = itemView.findViewById(R.id.card_note)

        fun bind(note: Note, mColor: String) {
            title.text = note.title
            main.setOnClickListener { callback(note) }
            main.setCardBackgroundColor(Color.parseColor("#$mColor"))
        }
    }
}